...
router.post('/payment/process', authMiddleware, async (req, res) => {
    try {
        ...
        if (status === 'success') {
            ...
            const discountAmount = (totalAmount * validDiscountPercent) / 100;
...
// Get UPI settings (accessible by all authenticated users)
router.get('/upi-settings', authMiddleware, async (req, res) => {
    try {
        const fs = require('fs');
        const path = require('path');
        const settingsPath = path.join(__dirname, '..', 'upi-settings.json');
        ...
        res.status(500).json({
            success: false,
            message: 'Error fetching UPI settings'
        });
    }
});

// Save UPI settings (admin only)
router.post('/upi-settings', authMiddleware, async (req, res) => {
    try {
        // Check if user is admin
        if (req.user.role !== 'admin') {
            return res.status(403).json({
                success: false,
                message: 'Only admin can update UPI settings'
            });
        }
        ...
        fs.writeFileSync(settingsPath, JSON.stringify({ upiId: upiId.trim() }, null, 2));
        ...
        console.error('Save UPI settings error:', error);
        res.status(500).json({
            success: false,
            message: 'Error saving UPI settings'
        });
    }
});
